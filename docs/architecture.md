...
This is contributor-facing design documentation. The user-facing entry point is
[README.md](../README.md).
...
Everything else is opt-in. These settings narrow it:
...
| **Predict Ethereum blocks** (alpha) | Restores the conservative rules — see [Predict Ethereum blocks](#predict-ethereum-blocks-alpha). |
...
year, currency, unit, date and hyphenated-identifier filters. Terse blocks lose
out either way: "thoughts on 8141?" scores 0 and stays locked, which is what
[Looking up a number you select](#looking-up-a-number-you-select) is for.
...
size. `wxt.config.ts` sets `publicDir: 'src/public'`: WXT resolves `publicDir`
against the project root, not `srcDir`, so without that line the manifest
references `icon/*.png` that never reach the bundle and Chrome refuses to load
the extension.
...
[publishing runbook](chrome-web-store-publishing.md) for setup, preflight,
...
then run in an idle callback). During fast continuous scrolling there is a brief
window where fresh references are not yet underlined. Measured on a synthetic
timeline appending 250 posts per batch:
...
measured above that is roughly 6000 posts. Feeds that drop offscreen nodes
