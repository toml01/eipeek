/**
 * Offline/manual signer for the deterministic payload reconstructed in memory.
 *
...
assertGeneratedDatabaseConstants(database);
const { payloadBytes } = database;
try {
...
const verified = await verifyDatabaseBytes(artifact, payloadBytes);
